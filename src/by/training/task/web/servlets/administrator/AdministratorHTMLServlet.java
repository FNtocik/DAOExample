...
@WebServlet(value = "/adminAdministrator", name = "administrator")
public class AdministratorHTMLServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        controls += allButtons;
        html = html.replace("$title", "administrator");
        html = html.replace("$header", headers);
        html = html.replace("$control", controls);
        html = html.replace("$scriptName", "admin/" + getServletName());
        html = html.replace("$localeName", "administrator");
