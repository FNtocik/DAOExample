...
@WebServlet(value = "/readerAdministrator", name = "readerAdministrator")
public class AdministratorHTMLServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        controls += allButtons;
        html = html.replace("$title", "reader");
        html = html.replace("$header", headers);
        html = html.replace("$control", controls);
        html = html.replace("$scriptName", "reader/" + getServletName());
        html = html.replace("$localeName", "reader");
